...
def austax(gross):
    ...
    if gross < 0 or gross > 10**20: raise ValueError("outside of bounds 0 - 10**20")
    ...
    hecs_net = gross - tax - hecs
    hecs_percent = round(100*hecs_percent, 2)
    
    #Output
    result = ["Gross: $", gross, ", Net: $", int(net), ", Tax: $", int(tax), ", Tax Percent: ", tax_percent, "%", ", HECS Net: $", int(hecs_net), ", HECS: $", int(hecs), ", HECS Percent: ", hecs_percent, "%"]
...
def home():
    if request.method == "GET":
        ...
        if gross.isnumeric():
            for item in austax(int(gross)).split(", "):
                if "HECS" in item:
                    if is_hecs:
                        flash(item)
                else: flash(item)
...
if __name__ == '__main__':
    app.run(host="10.0.0.55", port=80)